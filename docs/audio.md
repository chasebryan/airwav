...
- Install one supported output tool: `paplay` (PulseAudio / PipeWire compatibility — preferred), `pw-cat` (PipeWire), `aplay` (ALSA), or `ffplay`. Listen writes a 48 kHz mono streaming WAV header, then PCM. That is what libsndfile expects on stdin; sending raw samples made `pw-cat` report `sndfile: failed to open audio file "-" Format not recognised` and exit with Broken pipe. Raw fallbacks (`pw-cat -a`, `paplay --raw`, `aplay -t raw`) are tried if a WAV player dies. `pw-cat` is skipped when no PipeWire runtime socket is present so a hanging PipeWire client cannot block Pulse `paplay`. An installed but failing player reports its error and the next tool is tried. Select and unmute the default system output in your desktop sound settings.
