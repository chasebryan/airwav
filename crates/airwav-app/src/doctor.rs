//! Hardware and environment checks for `airwav doctor`.
use crate::doctor_storage;
...
pub(crate) fn doctor(
    ...
    let storage = doctor_storage::evaluate(data, available, config.minimum_free_bytes);
    add("storage", storage.status, storage.detail);
    add(
        "audio",
        "INFO",
